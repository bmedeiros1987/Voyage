...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    if (req.method === 'POST' && path === '/api/v1/availability/preview') return json(res, 200, buildAvailabilitySummary(await readJson(req, MAX_JSON_BYTES)));
    if (req.method === 'GET' && path === '/api/v1/planner/capabilities') return json(res, 200, plannerCapabilities());
    if (req.method === 'GET' && path === '/api/v1/planner/brain/capabilities') return json(res, 200, plannerBrainCapabilities());
    if (req.method === 'POST' && path === '/api/v1/planner/brain/brief') return json(res, 200, buildPlanningBrief(await readJson(req, MAX_JSON_BYTES)));
    if (req.method === 'POST' && path === '/api/v1/planner/brain/strategy') return json(res, 200, buildPlanningStrategy(await readJson(req, MAX_JSON_BYTES)));
    if (req.method === 'POST' && path === '/api/v1/planner/brain/assess') return json(res, 200, assessPlanQuality(await readJson(req, MAX_JSON_BYTES)));
    if (req.method === 'POST' && path === '/api/v1/planner/brain/replan') return json(res, 200, buildReplanDecision(await readJson(req, MAX_JSON_BYTES)));
    if (req.method === 'POST' && path === '/api/v1/planner/brain/learning-event') return json(res, 200, buildPreferenceLearningEvent(await readJson(req, MAX_JSON_BYTES)));
    if (req.method === 'POST' && path === '/api/v1/planner/preview') { const body = await readJson(req, MAX_JSON_BYTES); const brief = buildPlanningBrief(body); return json(res, 200, { ...buildAutomaticPlan(body), brain: { brief, strategy: buildPlanningStrategy({ ...body, brief }) } }); }
    if (req.method === 'POST' && path === '/api/v1/planner/external-itinerary/preview') return json(res, 200, normalizeExternalItinerary(await readJson(req, MAX_JSON_BYTES)));
    if (req.method === 'GET' && path === '/api/v1/planner/collaboration/capabilities') return json(res, 200, collaborationCapabilities());
    if (req.method === 'POST' && path === '/api/v1/planner/export/manifest') return json(res, 200, buildExportManifest(await readJson(req, MAX_JSON_BYTES)));
    if (req.method === 'GET' && path === '/api/v1/dietary/capabilities') return json(res, 200, dietaryCapabilities());
    ...
    const statusCode = statusForError(error); const errorCode = publicErrorCode(error);
    console.error(JSON.stringify({ level: statusCode >= 500 ? 'error' : 'warn', event: 'request_failed', requestId, errorCode, message: error instanceof Error ? error.message : 'unknown_error' }));
    return json(res, statusCode, { error: errorCode, requestId });
  } finally {
    console.log(JSON.stringify({ level: 'info', event: 'request_complete', requestId, method: req.method, path, durationMs: Date.now() - startedAt, statusCode: res.statusCode }));
  }
});

server.listen(config.port, '0.0.0.0', () => console.log(JSON.stringify({ level: 'info', event: 'server_started', service: 'voyage-api', port: config.port, environment: config.nodeEnv })));
...
function buildStaticMap() {
  ...
  const resourceFiles = [
    ...
    ['splash.svg', 'image/svg+xml; charset=utf-8']
  ];
  const map = new Map();
  for (const [name, contentType] of webFiles) {
    const entry = { url: new URL(`../app/www/${name}`, import.meta.url), contentType, name };
    map.set(`/${name}`, entry);
    map.set(`/voyage/${name}`, entry);
  }
  for (const [name, contentType] of resourceFiles) {
    const entry = { url: new URL(`../app/resources/${name}`, import.meta.url), contentType, name };
...
function requireContentType(req, expected) { const contentType = String(req.headers['content-type'] || '').split(';')[0].trim().toLowerCase(); if (contentType !== expected) throw namedError('unsupported_content_type'); }
