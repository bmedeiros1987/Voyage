...
export function authProviderCapabilities() {
  return Object.freeze({
    version: '1.0', providers: SUPPORTED_PROVIDERS, googleLoginScopes: GOOGLE_LOGIN_SCOPES, gmailImportScopes: GMAIL_IMPORT_SCOPES,
    principles: [
...
export function normalizeProviderIdentity(input = {}) {
  const provider = SUPPORTED_PROVIDERS.includes(String(input.provider || '').toUpperCase()) ? String(input.provider).toUpperCase() : null;
  ...
  return Object.freeze({
    ...
    grantedScopes: Object.freeze((Array.isArray(input.grantedScopes) ? input.grantedScopes : []).map(String).slice(0, 20))
  });
}

export function assertLoginScopeSeparation(identity) {
  const gmailInLogin = (identity?.grantedScopes || []).some((scope) => GMAIL_IMPORT_SCOPES.includes(scope));
  return Object.freeze({ ok: !gmailInLogin, reason: gmailInLogin ? 'GMAIL_SCOPE_MUST_NOT_RIDE_ON_LOGIN' : null });
}

export function evaluateAccountLink({ existingIdentity = null, incoming = null, userConfirmedLink = false } = {}) {
  const candidate = incoming && incoming.provider ? normalizeProviderIdentity(incoming) : null;
  if (!candidate) return Object.freeze({ decision: 'REJECT', reason: 'INVALID_PROVIDER_IDENTITY', linked: false });
  const scopeCheck = assertLoginScopeSeparation(candidate);
  if (!scopeCheck.ok) return Object.freeze({ decision: 'REJECT', reason: scopeCheck.reason, linked: false });
  if (!existingIdentity) return Object.freeze({ decision: 'CREATE_NEW_IDENTITY', reason: null, linked: false });
  const sameSubject = existingIdentity.provider === candidate.provider && existingIdentity.subject === candidate.subject;
  if (sameSubject) return Object.freeze({ decision: 'LINK', reason: 'VERIFIED_PROVIDER_SUBJECT_MATCH', linked: true });
  const emailMatches = Boolean(candidate.email) && candidate.email === existingIdentity.email;
  ...
  if (emailMatches) {
    return Object.freeze({ decision: 'REQUIRE_EXPLICIT_CONFIRMATION', reason: candidate.emailVerified ? 'EMAIL_ALONE_IS_NOT_PROOF' : 'EMAIL_NOT_VERIFIED_BY_PROVIDER', linked: false });
  }
