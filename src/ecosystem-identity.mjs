...
// Ecosystem identity: one globalUserId per human, with independent product
// memberships. Voyage never asks "are you crew?" — the backend resolves a
// CrewCheck membership silently from the linked account graph, and CrewCheck
// operational data never merges into the Voyage personal profile.
...
export function ecosystemIdentityCapabilities() {
  return Object.freeze({
    ...
    voyagePersonalScope: VOYAGE_PERSONAL_SCOPE,
    crewCheckOperationalScope: CREWCHECK_OPERATIONAL_SCOPE,
    principles: [
...
export function createGlobalIdentity({ globalUserId = null, createdAt = null } = {}) {
  return Object.freeze({
    globalUserId: globalUserId || `gid_${randomUUID()}`,
    createdAt: createdAt || new Date().toISOString()
  });
}

export function normalizeMembership(input = {}) {
  const product = PRODUCTS.includes(String(input.product || '').toUpperCase())
    ? String(input.product).toUpperCase()
    : null;
  if (!product) return null;

  // A membership is ACTIVE only when the product itself vouched for it. Anything
  // weaker (a marketing visit, an unverified claim) degrades to VISITOR.
  const verified = input.verifiedByProduct === true;
  const state = verified && input.active === true
    ? 'ACTIVE'
    : input.seen === true || verified ? 'VISITOR' : 'NONE';

  return Object.freeze({
    product,
    state,
    verifiedByProduct: verified,
    productAccountId: input.productAccountId ? String(input.productAccountId).slice(0, 190) : null,
    linkedAt: input.linkedAt || null,
    // Only CrewCheck can assert a crew role, and only on a verified active membership.
    crewRole: product === 'CREWCHECK' && verified && input.active === true && input.crewRole
      ? String(input.crewRole).toUpperCase().slice(0, 40)
      : null
  });
}
...
export function buildEcosystemIdentity(input = {}) {
  ...
  return Object.freeze({
    ...
    // Silent detection. This is a backend-only signal; the UI must not turn it
    // into a question, and a VISITOR must never be treated as a member.
    crewCheckDetection: Object.freeze({
      linkedAccountDetected: crewcheck.state !== 'NONE',
      activeMember: crewcheck.state === 'ACTIVE',
      crewRole: crewcheck.crewRole,
      detectionMethod: crewcheck.state === 'NONE' ? null : 'SILENT_IDENTITY_GRAPH',
      mustNotPromptUser: true
    }),
    dataBoundary: Object.freeze({
      voyageOwns: VOYAGE_PERSONAL_SCOPE,
      crewCheckOwns: CREWCHECK_OPERATIONAL_SCOPE,
      sharedByDefault: Object.freeze([]),
      requiresExplicitConnectionConsent: true
    })
  });
}

export function isCrewCheckMember(identity) {
  return identity?.memberships?.CREWCHECK?.state === 'ACTIVE';
}

export function assertNoOperationalLeak(voyageRecord = {}) {
  const leaked = Object.keys(voyageRecord).filter((key) =>
    CREWCHECK_OPERATIONAL_SCOPE.includes(String(key).toUpperCase()));
