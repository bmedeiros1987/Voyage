...
queueMicrotask(() => installUniversalImporterEnhancements().catch(() => {}));
...
async function installUniversalImporterEnhancements() {
  ...
  window.addEventListener('online', syncManualItems);
...
function injectReviewDialog() {
  ...
  dialog.innerHTML = `
    <form method="dialog" class="import-dialog__card" data-import-form>
      ...
      <div class="import-dialog__actions"><button class="button button--ghost" type="button" data-import-dialog-close>Cancelar</button><button class="button button--gold" type="submit">Salvar na jornada</button></div>
    </form>`;
...
async function handleEnhancementSubmit(event) {
  ...
  document.querySelector('[data-import-dialog]')?.close();
  await refreshQueueViaScreen();
  toast(existing ? 'Revisão salva.' : 'Item adicionado à jornada.');
}
...
function openEditor(record) {
  ...
  form.elements.location.value = payload.location || '';
  form.elements.provider.value = payload.provider || '';
  form.elements.confirmationCode.value = payload.confirmationCode || '';
  form.elements.notes.value = payload.notes || '';
  dialog.querySelector('[data-dialog-kicker]').textContent = record ? 'REVISÃO DO IMPORTADOR' : 'ITEM DA JORNADA';
  dialog.querySelector('[data-dialog-title]').textContent = record ? 'Confirmar dados' : 'Adicionar item';
...
function renderRecord(record) {
  ...
  const subtitle = isManual ? `${category} · item manual · ${formatDate(record.createdAt)}` : `${category} · ${formatBytes(record.size)} · ${formatDate(record.createdAt)}`;
...
function toast(message) {
  const el = document.querySelector('[data-import-toast]');
  if (!el) return;
  el.textContent = message;
  el.classList.add('import-toast--show');
  clearTimeout(toast.timer);
  toast.timer = setTimeout(() => el.classList.remove('import-toast--show'), 3400);
}
