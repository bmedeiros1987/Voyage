...
test('SQL journey data and sessions survive pool recreation and retain owner isolation', { skip: !databaseUrl }, async () => {
  ...
  try {
    for (const name of ['006_auth_sessions_itinerary_proposals.sql', '008_operational_journeys.sql']) {
    ...
    let store = createTidbPersistence({ execute: (...args) => pool.execute(...args) });
    ...
    store = createTidbPersistence({ execute: (...args) => pool.execute(...args) });
